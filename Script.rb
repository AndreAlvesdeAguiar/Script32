...
url_esp1 = URI('http://192.168.15.20/dados')  # ESP32 1
...
def save_data
  begin
    ...
    response_esp1 = Net::HTTP.get(url_esp1)
    data_esp1 = JSON.parse(response_esp1)
    puts "Dados recebidos do ESP32 1: #{data_esp1}"  # Verificação

    response_esp2 = Net::HTTP.get(url_esp2)
    data_esp2 = JSON.parse(response_esp2)
